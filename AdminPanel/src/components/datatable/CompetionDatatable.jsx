...
import { CompetitionColumns, TeamsColumns, userColumns } from "../../datatablesource";
...
const CompetitionDatatable = () => {
  ...
  const handleDelete = async (id) => {
    var config = {
      method: 'delete',
      url: `${process.env.REACT_APP_PROD_URL}delete/competition/${id}`,
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${localStorage.getItem("adminInfo").replace(/['"]/g, '')}`
      },
      ...
      renderCell: (params) => {
        return (
          <div className="cellAction">
            <Link to={`/competitions/single/${params.row.id}`} style={{ textDecoration: "none" }}>
  ...
  return (
    <div className="datatable">
      ...
      <div className="datatableTitle">
        Add New Team Member
        <Link to="/competitions/new" className="link">
          Add New
        </Link>
      </div>
      <DataGrid
        className="datagrid"
        rows={data}
        columns={CompetitionColumns.concat(actionColumn)}
        pageSize={9}
